...
function hysteresis(threshold, config) {
  ...
  a.number(threshold[0], 'hysteresis: Invalid threshold type. Lower value must be a number.')
  a.number(threshold[1], 'hysteresis: Invalid threshold type. Upper value must be a number.')
  a(threshold[0] < threshold[1], 'hysteresis: Invalid threshold. Lower cannot be less than upper.')
  a.object(config, 'hysteresis: Invalid config type. Config must be a plain object.')

  if (config.hasOwnProperty('initialSide')) {
    a.number(config.initialSide, 'hysteresis: Invalid initialSide config type: Must be number 0, 1, or not specified at all.')
    a((config.initialSide === 1 || config.initialSide === 0), 'hysteresis: Invalid initialSide config: must 0, 1 or not specified at all.')
  }
